...
async function login() {
  ...
  for (let i = 0; i < credentials.length; i++) {
    ...
    try {
      ...
      await driver.findElement(By.css('#resourceFieldContainer > div.level.colorLayer.good')).click();
      ...
      while (true) {
        try {
          // Find all upgradeable building slots
          let slots = await driver.findElements(By.css('#resourceFieldContainer > div.level.colorLayer.good'));
          if (slots.length === 0) {
